...
The script globs all T1w NIfTI files across all sessions for the given
subject. T2w images are also collected and, if present, passed with
``-T2``/``-T2pial``.  At least one T1w image is required.

In Apptainer mode the BIDS root and output directory are bind-mounted into the
container at ``/data`` and ``/output`` respectively; all file paths are
...
def collect_images(bids_dir: Path, subject: str) -> tuple[list[Path], list[Path]]:
    ...
    subdirectories.  Results include any contrast-entity variants
    (e.g. ``ce-corrected``, ``ce-uncorrected``) because they all match
...
def build_native_command(
    ...
    cmd = ["recon-all", "-subject", subject, "-sd", str(output_dir), "-all",
           "-parallel", "-openmp", str(threads)]
    for t1 in t1w_files:
        cmd += ["-i", str(t1)]
    if t2w_files:
...
def build_apptainer_command(
    ...
    cmd = [
        ...
        "--bind", f"{fs_license}:/opt/fs_license.txt:ro",
        str(sif),
        "recon-all",
        "-subject", subject,
        "-sd", "/output",
