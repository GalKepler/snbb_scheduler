...
This section describes the automated MRI processing pipeline implemented at the Strauss Neuroplasticity Brain Bank (SNBB). All processing stages were orchestrated by a custom rule-based scheduler that submitted jobs to a Slurm high-performance computing cluster, ensuring reproducibility and minimal manual intervention. The pipeline comprised seven sequential stages executed in dependency order: (1) DICOM to BIDS conversion, (2) post-conversion fieldmap processing, (3) anatomical image defacing, (4) diffusion MRI preprocessing, (5) voxel- and surface-based morphometry, (6) cortical surface reconstruction, and (7) diffusion MRI reconstruction and tractography. All neuroimaging tools were executed within Apptainer containers to guarantee version-locked, reproducible environments across compute nodes. Data were organized according to the Brain Imaging Data Structure (BIDS) specification throughout all stages (Gorgolewski et al., 2016).
...
## Voxel- and Surface-Based Morphometry — CAT12

Voxel- and surface-based morphometry were computed using the standalone build of the Computational Anatomy Toolbox (CAT12, CAT 26.0.rc3 r2874; Gaser et al., 2022), running on the free MATLAB Compiler Runtime and executed via Apptainer — no MATLAB license was required. This container is maintained by a separate companion project (`bagpipe`) that also uses it for cross-sectional brain-age inference; the scheduler's copy of the invocation is independently maintained so that the two projects' Slurm/cohort drivers do not share state. Processing was performed per session on the non-defaced, `rec-norm` T1-weighted image (the same selection rule used elsewhere in the pipeline, but explicitly independent of the defacing stage, since CAT12 output must remain comparable to the companion project's). CAT12 was run in classic (non-BIDS-redirect) output mode, producing tissue segmentations, ROI-based volumetric summaries across six built-in atlases (Neuromorphometrics, LPBA40, COBRA, Thalamus, Thalamic Nuclei, SUIT), plus the same custom volumetric parcellations used for the diffusion MRI reconstruction stage below (all Schaefer 2018 cortical + Tian 2020 subcortical resolution/scale combinations, HCPex, and the individual Tian subcortical scales — converted once from the shared atlas pack into CAT12's own ROI-atlas format), and surface-based cortical thickness, gyrification, sulcal depth, fractal dimension, and area estimates via CAT12's surface extraction module. Because the standalone binary always exits successfully regardless of internal failure, completion was independently verified from the presence of the segmentation report XML and the full set of surface-metric output files.
